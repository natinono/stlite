...
def get_kagaku():
    # 投資信託協会から最新の基準価額を取得
    fund_dict = {
        '国内株式': 'https://toushin-lib.fwg.ne.jp/FdsWeb/FDST030000/csv-file-download?isinCd=JP90C000FXV1&associFundCd=03311182',
        '先進国株式': 'https://toushin-lib.fwg.ne.jp/FdsWeb/FDST030000/csv-file-download?isinCd=JP90C000ENC5&associFundCd=03319172',
        '全世界株式': 'https://toushin-lib.fwg.ne.jp/FdsWeb/FDST030000/csv-file-download?isinCd=JP90C000H1T1&associFundCd=0331418A',
        '米国株式': 'https://toushin-lib.fwg.ne.jp/FdsWeb/FDST030000/csv-file-download?isinCd=JP90C000GKC6&associFundCd=03311187',
        'バランス': 'https://toushin-lib.fwg.ne.jp/FdsWeb/FDST030000/csv-file-download?isinCd=JP90C000EWV6&associFundCd=03312175',
    }
    ...
    for key, value in fund_dict.items():
        try:
            ...
            st.warning(f"{key}のデータ取得に失敗しました: {str(e)}")
    
    kagaku_df = kagaku_df[kagaku_df.index >= '2019-01-01']
    
    # 過去の基準価額をローカルファイル(CSV)から取得
    try:
        ...
        st.warning("過去データファイル(kagaku_past.csv)が見つかりません。2019年以降のデータのみを使用します。")
        df_kagaku = kagaku_df
...
def load_buy_data():
    ...
    try:
        ...
        st.error("購入履歴ファイル(toshin_buy.csv)が見つかりません。")
        return None
...
with st.sidebar:
    ...
    st.write("### 今後の月次投資額(万円)")
    senshinkoku_kongo = st.slider("先進国株式", 0, 40, int(senshinkoku_kongo/10000), 1) * 10000
    zensekai_kongo = st.slider("全世界株式", 0, 40, int(zensekai_kongo/10000), 1) * 10000
    beikoku_kongo = st.slider("米国株式", 0, 40, int(beikoku_kongo/10000), 1) * 10000
    kokunai_kongo = st.slider("国内株式", 0, 40, int(kokunai_kongo/10000), 1) * 10000
    balance_kongo = st.slider("バランス", 0, 40, int(balance_kongo/10000), 1) * 10000
    
    st.write("### 予測年利(%)")
    dict_growth['先進国株式'] = st.slider("先進国株式", 0, 20, dict_growth['先進国株式'], 1)
    dict_growth['全世界株式'] = st.slider("全世界株式", 0, 20, dict_growth['全世界株式'], 1)
    dict_growth['米国株式'] = st.slider("米国株式", 0, 20, dict_growth['米国株式'], 1)
    dict_growth['国内株式'] = st.slider("国内株式", 0, 20, dict_growth['国内株式'], 1)
    dict_growth['バランス'] = st.slider("バランス", 0, 20, dict_growth['バランス'], 1)

try:
    # データ読み込み
    with st.spinner('最新の基準価額を取得中...'):
    ...
    if first_row_index < len(df_buy):
        if '国内株式' in df_buy.columns:
            df_buy.iloc[first_row_index:, df_buy.columns.get_loc('国内株式')] = kokunai_kongo
        if '先進国株式' in df_buy.columns:
            df_buy.iloc[first_row_index:, df_buy.columns.get_loc('先進国株式')] = senshinkoku_kongo
        if 'バランス' in df_buy.columns:
            df_buy.iloc[first_row_index:, df_buy.columns.get_loc('バランス')] = balance_kongo
        if '全世界株式' in df_buy.columns:
            df_buy.iloc[first_row_index:, df_buy.columns.get_loc('全世界株式')] = zensekai_kongo
        if '米国株式' in df_buy.columns:
            df_buy.iloc[first_row_index:, df_buy.columns.get_loc('米国株式')] = beikoku_kongo
    ...
    fig4 = px.line(df_buy_hyouka[str_date2:today]['評価額'], title='直近期間の評価額推移')
    ...
    fig1 = px.line(df_kagaku_hiritu_filtered, title='基準価額推移(2017年末比)')
    ...
    trace2 = go.Scatter(x=monthly_profit_diff_df.index, y=monthly_profit_diff_df['12カ月移動平均'], name='12カ月移動平均', yaxis='y1', line=dict(color="#00008b"))
    trace3 = go.Scatter(x=monthly_profit_diff_df.index, y=monthly_profit_diff_df['24カ月移動平均'], name='24カ月移動平均', yaxis='y1', line=dict(color="#006400"))
    trace4 = go.Scatter(x=monthly_profit_diff_df.index, y=monthly_profit_diff_df['36カ月移動平均'], name='36カ月移動平均', yaxis='y1', line=dict(color="#ff0000"))
